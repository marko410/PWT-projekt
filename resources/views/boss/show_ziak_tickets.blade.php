...
@endif
        ...
        <div style='text-align:center'><a href="{{route('add_ziak_ticket')}}">
        <button class="btn btn-secondary">Pridať nový preukaz</button></a></div><br>
           

       <h2>Zoznam preukazov žiakov</h2><br>
...
if($go_hint>0){
  ...
  <span class="preukaz_minus" id="preukaz_minus_{{$ticket->id}}"><i class="fa fa-minus"></i></span>
      ...
      <form method="POST" action="{{route('delete_ziak_ticket_post')}}" id="delete_preukaz_{{$ticket->id}}">
      @csrf
...
<form method="POST" id='form_pilot' class="form_special" action="{{route('update_ziak_ticket_post')}}">
